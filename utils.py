...
class WeakSupervisorDataset(Dataset):
    def __init__(self, filename, max_seq_length, tokenizer, 
...
def write_weak_supervisor_predictions(all_predictions, final_prediction_file):
    # logger.info("Writing final predictions to: %s" % (final_prediction_file))
    output_dicts = []
    with open(final_prediction_file, 'w') as fout:
        for qid, preds in all_predictions.items():
...
def get_train_retriever_run(qids, pids, labels):
...
def weak_supervisor_eval(eval_file, model_output_file):
    weak_answer_dict = {}
